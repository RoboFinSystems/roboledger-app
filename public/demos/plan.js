 ...
 * in one monthly grid; a person changes one assumption (Summit Markets' payment
 * terms) and the forecast columns roll. Jun to Aug are Driftline's ledger; Sep
...
const main = `${pageHeader('Plan', 'Actuals and forecast in one grid, driven by the assumptions beneath')}
  <div class="scn"><span class="lbl">Scenario</span><span class="badge b-v" id="scn">Base</span></div>
  <table id="grid">
...
function setup(ctx) {
  ...
  ctx.nav('plan')
  return (t) => {
    const clicked = pointer(ctx, $('cur'), $('tv'), t, 1.4, 2.1)
    const val = clicked ? (t < 2.5 ? typed('30', t, 2.2, 8) || '' : '30') : '90'
    $('tv').textContent = val + ' days'
    $('terms').className = 'ar' + (clicked && t < 5.5 ? ' flash' : '')
