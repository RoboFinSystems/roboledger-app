...
export default function ModernConnectionsContent() {
  ...
  const loadConnections = useCallback(async () => {
    try {
      ...
      const qbData = qbConnections.data as ConnectionsResponse
      const plaidData = plaidConnections.data as ConnectionsResponse
      const secData = secConnections.data as ConnectionsResponse

      const allConnections = [
        ...(qbData?.connections || []).map(
          (conn: QuickBooksConnection, index: number) => ({
            id: `qb_${index}`,
            connection: {
              properties: {
        ...
        ...(plaidData?.connections || []).map(
          (conn: PlaidConnection, index: number) => ({
            id: `plaid_${index}`,
            connection: {
              properties: {
        ...
        ...(secData?.connections || []).map(
          (conn: SecConnection, index: number) => ({
            id: `sec_${index}`,
            connection: {
              properties: {
        ...
        {connections.map((connection, index) => {
          ...
          return (
            <ConnectionCard
              ...
              onSync={() =>
                handleSync(provider, connection.connection.properties.itemId)
              }
